...
| Concept | Meaning |
|---------|---------|
| **Hydration** | Near-zero session restore (`hydrateNearZeroSessionForWalletRoute` / `tryLoadNearZeroSessionIntoMemory`) plus bootstrap unlock (`orchestrateBootstrapUnlock` → per-rail **load**). Restore must invalidate the secrets-session probe and drop a stale successful bootstrap cache while the wallet is still gated. Distinct from background **sync** and **save**, which follow load and are not route-gated. |
