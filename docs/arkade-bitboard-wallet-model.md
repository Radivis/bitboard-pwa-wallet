...
**Proceed automatically:** Optional fire-and-forget mode on the control page. When enabled, the wallet re-resolves the selected Low/Medium/High preset each step (capped by a user-defined max sat/vB) and calls `ark_proceed_unilateral_exit_step` in a background runner (`ActiveWalletBootstrap`) while the app stays **unlocked** and the **same descriptor wallet** remains selected. This is **not** delegator-based — closing the tab stops automation. Automation pauses when the live preset exceeds the max cap, bumper balance is insufficient, or a step errors; job state is persisted per wallet in SQLite.
