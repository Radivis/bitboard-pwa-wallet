...
This is the planning parent. The work shipped together in the envelope **v8→v11** PR (`unilateral-exit-vtxo-lifecycle`). Current behavior lives in [unilateral-exit.md](../unilateral-exit.md); this file is the original design notes.
...
- Test contracts: `ARK-EXIT-*` in [doc/features/arkade.yaml](../../doc/features/arkade.yaml) (`ARK-EXIT-27`–`33` are the VTXO lifecycle; `01`–`26` remain shipped behavior)
...
| What | Decision |
|------|----------|
| Unified Esplora 6-conf reconciler | **Do** |
| Host-tx observation registry | **Do** |
...
**Complete without an in-progress membership test.** `complete_unilateral_exit` must not require the outpoint to be in `unilateral_exit_in_progress_outpoints()` (`VtxoNotInUnilateralExit`). The gate is snapshot readiness (`is_unrolled && !is_spent`, later `complete_ready`) plus Esplora claimable. Aborted or failed jobs may still leave VTXOs that should be claimed.

---

## Why this refactor

Historical motivation (these holes are closed). Abort and failure stop the **frontend job**. They do not un-broadcast anything (`ARK-EXIT-23`). Completing those coins still requires local `is_unrolled`, and that stamp originally ran only while the job was polling Esplora (`proceed` / `get_unilateral_exit_progress` for leaves; operator sync for intermediate hosts).
...
Complete also used to require membership in `unilateral_exit_in_progress_outpoints()`. Aborted-but-unrolled VTXOs must remain claimable without that job.

The lock was the Esplora reconciler, host-tx observations, and complete without in-progress membership — not a dedicated 6-conf poller. See [Design options](#design-options).
...
| **Host-tx observation** | virtual `txid` | Did we attempt broadcast? Relayed? Confirmations? Still in the Esplora hot set? |
...
| Host-tx confirmations | 0 / relayed / 1-conf / 6-conf | WASM Esplora reconciler + observation row |
...
**`never_seen` budget** (evaluated on Esplora reconcile call sites, not a sleep loop; freeze in [Agreed freeze](#agreed-freeze)): first eligible miss after `registered_at + 10 minutes`, then up to four more at `last_probed_at + 1 minute`. At most one probe increment per txid per reconcile call; do not collapse the budget across a time skip; 15s polls must not reset `last_probed_at` except on an eligible miss. This long wait is the cleanup of the early register when Esplora still has nothing — not a reaction to the first miss. If still absent:
...
**Materials heal:** on load/sync, also probe host txs of tagged VTXOs in materials even if an observation write was missed. The observation table is the cheap hot set; the reconciler is “if it is 6-conf on chain, stamp it.”

**Esplora reconcile call sites** (no dedicated poll actor): Arkade load (including autonomous), operator sync, proceed, progress, `list_unilateral_exits_in_progress`, complete. Same 6-conf constant as today. Skip `commitment` / `checkpoint`. Include terminal leaves (an intermediate-only reconciler is the wrong split).
...
**Spend-lock from `tagged` is a balance-model change.** During unroll those VTXOs stay in gross spendable; pending deductions are informational until 6 confs until records own the line. Target: `unilateral_exit_in_progress` includes `tagged` onward, and those sats are not collaboratively spendable. Renew / send / collab / delegate must refuse them. Recover and signer-migrate exclude in-progress pipeline only (not `funding_lost`). Closing the 6-conf safety hole must not silently change dashboard math; spend-lock from `tagged` is the records increment.

**Complete gate:** snapshot `complete_ready` (unrolled + unspent + timelock/claimable). No `VtxoNotInUnilateralExit` / in-progress membership.
...
|-------|----------------|
| `HYDRATE` | WASM record on load / after Esplora reconcile |
...
1. Spec job vs VTXO machines, events, spawn/stop rules.
2. Job machine must not be the only 6-conf stamper (no new VTXO machine required for that).
3. Optional: spawn children as a **read-only view** of WASM records (selectors). Do not move spend-lock into ad hoc React state.
...
Shipped: VTXO exit table + host-tx observation table are authoritative. Do not debug three truths.
...
| `unilateral_exit_watches` | Survival of `unrolled+` across snapshot replace — **folded into records** |
...
| Complete `VtxoNotInUnilateralExit` | Deleted |
...
Originally planned as separate PRs. They shipped together in envelope v8→v11. Exit criteria below are what later increments may assume.
...
- Esplora reconcile call sites; no dedicated 6-conf poller
- Abort rules and spend-lock from `tagged` (as **target**; implementation with VTXO records)
- Complete = `complete_ready`
- Job vs VTXO XState split

**Out of scope:** code, envelope version bump.

**Exit:** YAML (`ARK-EXIT-27`–`32` plus annotations on shipped `ARK-EXIT-*` / `ARK-REC-08`) and the [Agreed freeze](#agreed-freeze) below. This is documentation, not an implementation of the later increments.

### Agreed freeze

Spec IDs: `ARK-EXIT-27`–`32` in [doc/features/arkade.yaml](../../doc/features/arkade.yaml). Shipped `ARK-EXIT-01`–`26` stay current behavior until later increments implement the target.

#### Resolved “or”s

1. **`never_seen` deletes the observation.** When the budget fires, delete the row so the txid leaves the hot set. VTXOs stay `tagged` (until records exist: leave pending deductions). That rewind is the **delayed** cleanup of the pre-broadcast register after Esplora still has nothing (~14 minutes, five eligible misses) — not an immediate untag on broadcast error or first miss. Abort after that rewind may unlock. Rows kept as 6-conf evidence are still deleted when every VTXO on that host is `exited` or `funding_lost`. Re-proceed with the same deterministic txid **re-registers** and resets the probe window. `step_wait` does not veto this path.
2. **Probe budget is not collapsed across a time skip.** Evaluated only on Esplora reconcile call sites (no 6-conf `after` actor). At most one probe increment per txid per reconcile call. First miss counts only after `registered_at + 10 minutes`; later misses need `last_probed_at + 1 minute`. Five eligible misses (1 + 4) **delete** the row. A single load after an hour counts as one miss. Independent of this, if Esplora shows ≥6 confs, the reconciler still stamps (materials heal).
...
**Spend-lock from `tagged`** is the records increment. Closing the 6-conf safety hole must not change dashboard spendable math.

### Close the 6-conf safety hole

The original bug. Ship this even if later increments slip.
...
- Run the reconciler on load (including autonomous), operator sync, proceed, progress, list-in-progress, complete.
- `never_seen` probe budget; do not untag VTXOs (until records exist: leave pending deductions in place).
...
**Keep** pending deductions, watches, `is_unrolled`, job machine as they are. Observations plus the reconciler **feed** `is_unrolled` so aborted unrolls become completable.

**Out of scope:** spend-lock from job start, VTXO phase enum in UI, balance-table change, XState children, folding watches.

**Exit:** After abort (or killed job) a host tx that later reaches 6 confs is stamped on the next Esplora reconcile; Complete can claim it without a frontend job. Autonomous load stamps without operator sync. No new always-on poller.

### VTXO records as source of truth
...
### Fold watches and persist funding_lost
...
- Keep two stampers (leaf vs intermediate) with different callers after the unified reconciler ships.
- Poll 6-conf only while the job is in `waitingConfirm`.
- Add a dedicated unlocked-session 6-conf poller.
...
- Let React or hooks own VTXO phase outside XState once VTXO children exist.
- Re-split the already-shipped v8→v11 envelope into historical PRs. Keep follow-on work reviewable.

---

## Implementation-plan checklist

When writing a follow-on plan, include:
...
5. Explicit “does not change” list (especially dashboard balance when only closing the 6-conf hole)
