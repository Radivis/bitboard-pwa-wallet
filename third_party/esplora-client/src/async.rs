...
pub struct AsyncClient<S = DefaultSleeper> {
    ...
    max_retries: usize,
    /// On wasm32, reqwest has no client-level timeout; apply this to each request instead.
...
impl<S: Sleeper> AsyncClient<S> {
    ...
    pub fn from_builder(builder: Builder) -> Result<Self, Error> {
        let timeout_secs = builder.timeout;
        ...
        let request_timeout = timeout_secs.map(Duration::from_secs);
        #[cfg(not(target_arch = "wasm32"))]
        let request_timeout: Option<Duration> = None;
