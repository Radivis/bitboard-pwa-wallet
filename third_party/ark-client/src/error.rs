    ...
    fn is_version_mismatch(&self) -> bool {
        match self {
            Kind::ArkServer(err) => {
                ...
                {
                    err.source.is::<crate::ark_grpc_wasm_shim::Error>()
                        && err
                            .source
                            .downcast_ref::<crate::ark_grpc_wasm_shim::Error>()
                            .is_some_and(crate::ark_grpc_wasm_shim::Error::is_version_mismatch)
                }
            }
            Kind::AdHoc(_)
            | Kind::Core(_)
...
#[cfg(all(target_arch = "wasm32", target_os = "unknown"))]
impl From<crate::ark_grpc_wasm_shim::Error> for Error {
    fn from(value: crate::ark_grpc_wasm_shim::Error) -> Self {
        if value.is_server_info_changed() {
            Self::server_info_changed(Self::ark_server(value))
        } else {
            Self::ark_server(value)
