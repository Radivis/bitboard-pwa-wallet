...
| **Recent-script VTXO sync** | Incremental operator sync must query only the gap window, not every cached HD index. | `Client::derivation_index_for_pk` is public so Bitboard can filter `get_offchain_addresses()` by index. `peek_next_offchain_derivation_index` was already public. | `src/lib.rs` | `bitboard-ark` incremental VTXO merge unit tests |
