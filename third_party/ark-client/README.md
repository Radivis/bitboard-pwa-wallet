...
| **Completion coin-select** | After unroll, indexer marks VTXOs `is_spent`/`is_unrolled` so `all_unspent()` drops them; thin Esplora stacks often omit `confirmation_blocktime`. | `coin_select_vtxo_txids_for_onchain` searches `vtxo_list.all()`, allows missing blocktime with `Duration::ZERO` (surfaced via `VtxoCompletionSelection::missing_blocktime_inputs`). | `src/coin_select.rs` | `unilateral_exit_session_regtest` (`unilateral_unroll_and_complete_on_regtest`) |
