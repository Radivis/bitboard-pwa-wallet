...
| **Unilateral exit — completion fee** | Upstream hard-coded a 1 000 sat completion fee. | Build-and-measure fee targeting from Esplora fee rate (iterative vsize measurement). | `src/unilateral_exit.rs` | `unilateral_exit_session_regtest` |
| **Unilateral exit — indexer paging** | arkd rejects vtxo/chain queries without explicit positive `page.size`. | Paginated chain and virtual-tx fetches (page size 100). | `src/unilateral_exit.rs` | `unilateral_exit_session_regtest` |
| **Unilateral exit — commitment visibility** | Esplora may lag behind arkd’s round commitment broadcast. | Short poll loop before treating commitment as missing during unroll. | `src/unilateral_exit.rs` | `unilateral_exit_session_regtest` |
...
| **Boarding / batch settle** | arkd rejects stale boarding inputs past the cooperative window; boarding intents need the operator intent fee reserved in the offchain output. Post-register batch SSE waits 60s per event; timeout after `RegisterIntent` returns `JoinBatchOutcome::Waiting` instead of a failed join. | `is_past_arkd_cooperative_boarding_window`, `board_to_amount_after_intent_fee` / intent-fee evaluation in batch paths; `BATCH_EVENT_TIMEOUT`. | `src/batch.rs` | E2E arkade-regtest boarding flows; signer migration native boarding test |
| **Batch intent delete (WASM)** | Pending intents can wedge later rounds if client drops after register and before follow-up signing. arkd `deleteIntent` matches only VTXO `Inputs`, not `BoardingInputs`, so boarding-only cancel needs a local fallback after a valid proof miss. | Added `delete_registered_intent` using ownership-only proofs (`Delete` message, empty outputs, `expire_at: 0`). Boarding-only cancel clears local wait state when arkd returns `no matching intents found for intent proof`. | `src/batch.rs`, `src/ark_grpc_wasm_shim.rs`, `src/error.rs` | Vitest pending-intent Cancel/Retry flows |
