...
| **Batch intent delete (WASM)** | Pending intents can wedge later rounds if client drops after register and before follow-up signing. | Added `delete_registered_intent` intent-proof flow using `IntentMessage::Delete` over selected inputs on the WASM REST transport. | `src/batch.rs`, `src/ark_grpc_wasm_shim.rs` | Vitest pending-intent Cancel/Retry flows |
