...
| `VITE_E2E_ARKADE_MOCK` | When `true`, dev server serves mocked Ark operator responses for dashboard Arkade E2E (`npm run test:e2e:arkade`). Playwright sets a per-test partition cookie so parallel workers do not share mock state (operator traffic runs in the Arkade WASM worker, not through `page.route`). |
