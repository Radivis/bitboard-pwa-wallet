...
export function LabMempoolCard({
  ...
  return (
    <InfomodeWrapper
      ...
      infoText="The mempool lists transactions that have been created in the lab but not yet included in a block. After you mine blocks, those txs move into “Confirmed” with a confirmation count (how many blocks have been mined on top). Confirmed rows will move to per-block views later; for now they stay here. Tap any row to inspect inputs, outputs, and amounts in detail."
      className="rounded-xl"
    >
      <Card>
        <CardHeader>
          <CardTitle>Mempool</CardTitle>
          <CardDescription>Mempool and confirmed transactions (for now)</CardDescription>
        </CardHeader>
            ...
            {sortedTransactions.length === 0 && mempool.length === 0 ? (
              ...
              <div className="space-y-2">
                {sortedTransactions.slice(0, MAX_DISPLAYED_LAB_TRANSACTIONS).map((tx) => {
                  ...
                  const confirmations = details ? blockCount - details.blockHeight : 0
                  return (
                    <Link
                      key={tx.txid}
                      to="/lab/tx/$txid"
