...
/// Signs a transaction where each input may be P2WPKH or P2TR, controlled by different keys.
/// Uses `script_pubkey_to_wif_json` (e.g. `{"spk_hex":"wif"}`) to look up the WIF
/// for each input based on the UTXO's `script_pubkey_hex` field.
#[wasm_bindgen]
pub fn lab_sign_transaction_multi_by_script_pubkey(
    tx_hex: &str,
    utxos_json: &str,
    script_pubkey_to_wif_json: &str,
) -> Result<String, JsValue> {
    let mut tx: Transaction =
        deserialize_hex(tx_hex).map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    let script_pubkey_to_wif: std::collections::HashMap<String, String> =
        serde_json::from_str(script_pubkey_to_wif_json)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    for (i, utxo) in utxos.iter().enumerate() {
        ...
        let wif = script_pubkey_to_wif
            .get(&utxo.script_pubkey_hex)
            .cloned()
            .ok_or_else(|| {
                JsValue::from_str(&format!(
                    "Input {} (script_pubkey {}) not controlled by wallet",
                    i, utxo.script_pubkey_hex
                ))
            })?;
